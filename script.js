...
async function fetchAndCacheEmojis() {
    ...
    if (!res.error && Array.isArray(res)) {
        res.forEach(emoji => {
            globalEmojiMap.set(emoji.id, { preview: emoji.urlPreview, anim: emoji.urlAnimation }); 
        });
    }
}
...
window.showQuestModal = (questId) => {
    ...
    // Removed vote section for wiki purposes
    // We check if it's an available quest in clan to show votes, but for generic wiki we hide it.
    let votesHtml = '';
    // Only show votes if we have data for this quest ID (implies we are in clan view context, not just wiki)
    if (clanVotesCache.votes && clanVotesCache.votes[questId]) {
        const voterIds = clanVotesCache.votes[questId];
        if (voterIds.length > 0) {
            const voteList = voterIds.map(vid => {
                const name = clanMembersCache[vid] || 'Unknown';
                return `<span class="voter-tag">${name}</span>`;
            }).join('');
            votesHtml = `<h4 style="margin:15px 0 10px 0; color:#334155;">🗳️ Votes (${voterIds.length})</h4><div style="margin-top:5px;">${voteList}</div>`;
        } else {
             votesHtml = `<h4 style="margin:15px 0 10px 0; color:#334155;">🗳️ Votes</h4><p style="color:#64748b;">No votes yet</p>`;
        }
    }

    // If accessed from Wiki, we might want to hide votes completely if it's not relevant
    // Based on user request "เอาส่วนโหวดออกด้วยในหน้าเควสทั้งหมด" -> hide votes in all quests page
    // So if we don't have vote data, we don't show the header at all.
...
// ... (Other functions remain mostly the same, ensuring full implementation)
...
// **********************************************
// 8. CLAN DATA FETCHING & RENDERING
// **********************************************
async function fetchMyClan() {
    if (!localStorage.getItem('wolvesville_api_key')) return showCustomAlert('Error', 'Missing API Key');
    stopClanPolling();
    document.getElementById('clan-content-container').innerHTML = '<div class="loading-container"><div style="font-size:24px;">🛡️</div><h3>Loading...</h3></div>';
...
async function searchClan() {
    ...
    if (!name) return;
    document.getElementById('clan-content-container').innerHTML = '<div class="loading-container"><div style="font-size:24px;">🔍</div><h3>Searching...</h3></div>';
...
async function fetchClanData(clanId, isMyClan = false, isBackground = false) {
    if(!isBackground) {
        isFirstRender = true;
        document.getElementById('clan-content-container').innerHTML = '<div class="loading-container"><h3>Loading Data...</h3></div>';
    }
    ...
    if (info.error) { 
        if(!isBackground) document.getElementById('clan-content-container').innerHTML = `<div style="text-align:center; color:red;">Error: ${info.message}</div>`;
        return; 
    }
...
function renderClanDashboard(info, members, quests, chat, logs, ledger, history, announcements, blockedMembers, availableQuests, votesData, clanId, canEdit = false, isBackground = false, participatingMemberCount = 0) { 
    ...
    safeMembers.forEach(m => { memberMap[m.playerId] = m.username; });
    ...
    if (!quests.error && (quests.quest || (quests.id && (quests.promoImageUrl || quests.rewards)))) {
        ...
        const actionCost = 300 + (30 * safeMembers.filter(m => m.participateInClanQuests).length);
    ...
    if (canEdit && !availableQuests.error && Array.isArray(availableQuests) && availableQuests.length > 0) {
        ...
        if (!votesData.error && votesData.shuffleVotes && votesData.shuffleVotes.length > 0) {
    ...
    if (!members.error && Array.isArray(safeMembers)) {
        membersHtml = safeMembers.map(m => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(saveApiKeyBtn) saveApiKeyBtn.addEventListener('click', () => { const v = apiKeyInput.value.trim(); if(v.length>10) { localStorage.setItem('wolvesville_api_key',v); alert('Saved!'); fetchAndDisplayData(); } else alert('Key too short'); });
